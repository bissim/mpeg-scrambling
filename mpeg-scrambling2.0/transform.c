...
PrintMatrixN4M() prints an nxm matrix in row/column form. 

EFUNC*/

void PrintMatrixN4M(int* matrix, int n, int m)
{
  BEGIN("PrintMatrixN4M");
  int i,j;
  ...
  if (matrix)
    {
      for(i=0;i<n;i++)
	{
	  for(j=0;j<m;j++) {printf("%6d ",*(matrix++));}
	  printf("\n");
	}
    }
  else {printf("Null\n");}
...
void CreateFileMatrix(int* matrix, int n, int m, char * filename)
{
  BEGIN("CreateFileMatrix");
  ...
  if (matrix)
	...
	{
	  for(j=0;j<m;j++) {
            if(j < m-1)fprintf(f, "%d,", *(matrix++)); else fprintf(f, "%d\r\n", *(matrix++));
          }
	}
      fclose(f);
    }
